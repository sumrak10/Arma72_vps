...
async def notify_new_order(order: OrderSchema):
    ...
    for product in order.products:
        text += f"⭕️ \
<a href='https://arma72.com/admin/shop/product/{product.id}/change/'>{product.name}</a>\n\
- Кол-во: {product.count}\n- Сумма: {product.summ}\n"
        if product.options != "None":
            text += f"- Опция: {product.options}\n"
        text += "\n"
    text += f"👁 <a href='https://arma72.com/admin/CRM/order/{order.id}/change/'>\
Посмотреть в админ панели</a>"
